...
{System.out.println("[Sintáctico] [Linea " + Lexico.linea + "] {Declaración de función de definición de tipo llamada " + yyvsp[-4].sval +"'}");
    	        if (yyvsp[-1].sval.equals(yyvsp[-4].sval)){
                    ...
                    Main.listaErrores.add("[ERROR SEMÁNTICO] [Linea " + Lexico.linea + "] {" + yyvsp[-5].sval + " y " + yyvsp[-1].sval +" deben tener el mismo nombre}");
...
{System.out.println("[Sintáctico] [Linea " + Lexico.linea + "] {Asignacion del repeat: "+ yyvsp[-2].sval + " := "+ yyvsp[0].sval + "}");
                    ...
                    if(ambitoVariable != null) {
                        ...
                        Main.listaErrores.add("[ERROR SEMÁNTICO] [Linea " + Lexico.linea + "] {La variable " + yyvsp[-2].sval +" no fue declarada}");
                    }
break;
case 51:
...
{System.out.println("[Sintáctico] [Linea " + Lexico.linea + "] {Asignacion de " + yyvsp[-2].sval +"}");
            String ambitoVariable = Main.tablaSimbolos.verificarAmbito(yyvsp[-2].sval, ambito);
            /*TODO MUCHOS ELSE*/
            if(ambitoVariable != null){
                String tipoIdentificador = Main.tablaSimbolos.getDatos(ambitoVariable).getTipo();
               ...
               if (yyvsp[-1].sval != null){
                    ...
                    if(ambitoFuncion != null && Main.tablaSimbolos.getDatos(ambitoFuncion).getTipo() == "ULONG"){
                        ...
                        if(ambitoFuncion == null){
                            Main.listaErrores.add("[ERROR SEMÁNTICO] [Linea " + Lexico.linea+ "] {La función "+yyvsp[-3].sval+" esta fuera de alcance}");
                            yyval = new ParserVal(null);
                        }
                        else{
                            Main.listaErrores.add("[ERROR SEMÁNTICO] [Linea " + Lexico.linea+ "] {Se invocó a la función "+yyvsp[-3].sval+" con un parámetro de otro tipo}");
                            yyval = new ParserVal(null);
                        }
               }
            }
                 ...
                 if (yyvsp[-1].sval != null && yyvsp[-3].sval != null){
                    ...
                    if(ambitoFuncion != null){
                        if (ambitoParametro != null){
                             ...
                             Main.listaErrores.add("[ERROR SEMÁNTICO] [Linea " + Lexico.linea+ "] {El parámetro "+yyvsp[-1].sval+" esta fuera de alcance o no esta declarado}");
                        }
                    }
                    else{
                           Main.listaErrores.add("[ERROR SEMÁNTICO] [Linea " + Lexico.linea+ "] {La función "+yyvsp[-3].sval+" esta fuera de alcance}");
...
{System.out.println("[Sintáctico] [Linea " + Lexico.linea + "] {Declaración de función llamada '"+ yyvsp[-1].sval +"'" );
...
#line 556 "Gramatica.y"
{
                    ...
                    if (!parametroCopiaValor.equals("")){
                        ...
                        if (!Main.tablaSimbolos.existeLexema(nuevoLexema)){
                            ...
                            Main.listaErrores.add("[ERROR SEMÁNTICO] [Linea " + Lexico.linea + "] {La funcion '" + yyvsp[-3].sval + "' ya fue declarada en este ámbito}");
...
{System.out.println("[Sintáctico] [Linea " + Lexico.linea + "] {Identificador: " + yyvsp[0].sval +"}");
	            String ambitoVariable = Main.tablaSimbolos.verificarAmbito(yyvsp[0].sval, ambito);
                	if(ambitoVariable != null)
                        ...
                        Main.listaErrores.add("[ERROR SEMÁNTICO] [Linea " + Lexico.linea + "] {La variable " + yyvsp[0].sval +" no fue declarada}");
                        yyval = new ParserVal(null);
	                }
	          }
break;
